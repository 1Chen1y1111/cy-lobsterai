...
const McpServerFormModal: React.FC<McpServerFormModalProps> = ({ isOpen, server, registryEntry, existingNames, onClose, onSave }) => {
  const isEdit = !!server
  ...
  const kvInputClass =
    'flex-1 px-2 py-1.5 text-sm rounded-lg dark:bg-claude-darkBg bg-claude-bg dark:text-claude-darkText text-claude-text border dark:border-claude-darkBorder border-claude-border focus:outline-none focus:ring-1 focus:ring-claude-accent'
  ...
  // Save button text
  const saveText = isRegistry && !isEdit ? i18nService.t('mcpInstall') : i18nService.t('saveMcpServer')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="text-lg font-semibold dark:text-claude-darkText text-claude-text">{modalTitle}</div>
        </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className={labelClass}>
                    ...
                    {isRegistry && envRows.some((r) => r.required) && (
                      <span className="ml-2 text-[10px] text-red-400 font-normal">* {i18nService.t('mcpRequiredConfig')}</span>
                    )}
                  </label>
                  <button
                    type="button"
                ...
                {envRows.map((row, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    {row.required && <span className="text-red-400 text-xs flex-shrink-0 w-4 text-center">*</span>}
                  </div>
                ))}
              </div>
            </>
