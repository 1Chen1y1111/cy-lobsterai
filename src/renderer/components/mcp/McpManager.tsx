...
const McpManager: React.FC = () => {
  ...
  const [dynamicCategories, setDynamicCategories] =
    useState<ReadonlyArray<{ id: string; key: string; name_zh?: string; name_en?: string }>>(mcpCategories)
  ...
  const filteredInstalled = useMemo(() => {
    ...
    return servers.filter(
      (server) => server.name.toLowerCase().includes(query) || getInstalledDescription(server).toLowerCase().includes(query)
    )
  ...
  const filteredMarketplace = useMemo(() => {
    ...
    if (query) {
      entries = entries.filter((e) => e.name.toLowerCase().includes(query) || getRegistryEntryDescription(e).toLowerCase().includes(query))
    }
  ...
  const filteredCustom = useMemo(() => {
    ...
    return custom.filter((s) => s.name.toLowerCase().includes(query) || s.description.toLowerCase().includes(query))
  }, [servers, searchQuery])
  ...
  const tabIndicatorClass = (tab: McpTab) =>
    `absolute bottom-0 left-0 right-0 h-0.5 rounded-full transition-colors ${activeTab === tab ? 'bg-claude-accent' : 'bg-transparent'}`

  return (
    <div className="space-y-4">
      {/* Description */}
      <p className="text-sm dark:text-claude-darkTextSecondary text-claude-textSecondary">{i18nService.t('mcpDescription')}</p>
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-sm font-medium dark:text-claude-darkText text-claude-text truncate">{server.name}</span>
                      </div>
                    ...
                    <Tooltip content={installedDescription} position="bottom" maxWidth="360px" className="block w-full">
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-sm font-medium dark:text-claude-darkText text-claude-text truncate">{entry.name}</span>
                      </div>
                      <div className="flex items-center gap-1.5 flex-shrink-0">
      ...
      {pendingDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={handleCancelDelete}>
          <div
          ...
          >
            <div className="text-lg font-semibold dark:text-claude-darkText text-claude-text">{i18nService.t('deleteMcpServer')}</div>
